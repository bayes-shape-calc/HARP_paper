...
if __name__ == '__main__':
	...
	parser.add_argument('step', type=str, choices=['download','calculate','radial','metadata','progress','check','results','everything','remove_empty'], help='Step of the paper to run. Using everything runs: <download, calculate, metadata, alignment, progress, collect>')
	...
	if args.step in ['calculate','everything']:
		...
		harp_paper.calc_blobornot(args.data_dir,args.results_dir,args.cutoff,args.nworkers,args.allow_big)

	if args.step in ['radial']:
		print('Beginning radial calculations of <%.2fA structures from %s into %s'%(args.cutoff,args.data_dir,args.results_dir))
		harp_paper.calc_radial(args.data_dir,args.results_dir,args.cutoff,args.nworkers,args.allow_big)
