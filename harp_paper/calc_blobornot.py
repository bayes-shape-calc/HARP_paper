...
def calc_blobornot(fdir,out_dir,cutoff,num_workers,allow_big=False):
	t0 = time.time()
		...
		while len(jobs) < num_workers:
			if len(pdbids) > 0:
				...
				if not check_results(os.path.join(out_dir,'result_%s.csv'%(pdbid))):
					...
					p = multiprocessing.Process(target=run_em_pdbid, args=(pdbid,fdir,job_i,job_total,out_dir,allow_big))
