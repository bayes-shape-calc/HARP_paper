...
for cutoff_resolution in [[0.,2.],[2.,2.5],[2.5,2.75],[2.75,3.],[3.,3.25],[3.25,3.5],[3.5,3.75],[3.75,4.]]:
	...
	for i in tqdm(range(len(deposit_date))):
		...
		if int(y) >= cutoff_year:
			if resolution[i] >= cutoff_resolution[0] and resolution[i] <=cutoff_resolution[1]:
				pp = P_res[i].copy()
				pp[~np.isfinite(pp)] = 0.
				pp[np.isnan(pp)] = 0.
	...
	####
	print('****** %.2f-%.2f ******'%(cutoff_resolution[0],cutoff_resolution[1]))
	...
	fig.savefig('figures/rendered/EPres_residue_range_aa_%.2f-%.2f.png'%(cutoff_resolution[0],cutoff_resolution[1]),dpi=300)
	fig.savefig('figures/rendered/EPres_residue_range_aa_%.2f-%.2f.pdf'%(cutoff_resolution[0],cutoff_resolution[1]))
	plt.close()
